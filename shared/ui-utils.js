...
export function isWriteTabActive() {
  const allWriteTabs = document.querySelectorAll(
    GITHUB_SELECTORS.WRITE_TAB.ALL,
  );
  return Array.from(allWriteTabs).some(
    (tab) =>
...
export function isPreviewTabActive() {
  const allPreviewTabs = document.querySelectorAll(
    GITHUB_SELECTORS.PREVIEW_TAB.ALL,
  );
  return Array.from(allPreviewTabs).some(
    (tab) =>
      tab.classList.contains("selected") ||
      tab.getAttribute("aria-selected") === "true",
  );
...
export function getPRDescriptionTextarea() {
  return (
    document.querySelector(GITHUB_SELECTORS.PR_DESCRIPTION) ||
...
export function isRelevantMutation(mutation) {
    ...
    addedNodes.some(
      (node) =>
        ...
        (node.nodeType === Node.ELEMENT_NODE &&
          node.querySelector(`#${BUTTON_CONFIG.ID}`)),
    ) ||
    removedNodes.some(
      (node) =>
        node.id === BUTTON_CONFIG.ID ||
  ...
  return (
    addedNodes.some((node) => {
      ...
      return (
        node.matches &&
        GITHUB_SELECTORS.MUTATION_TARGETS.some(
    ...
    removedNodes.some((node) => {
      ...
      return (
        node.matches &&
        GITHUB_SELECTORS.MUTATION_TARGETS.some((selector) =>
...
export function showNotification(
  message,
  type = "info",
  duration = NOTIFICATION_STYLES.DURATION,
) {
 ...
 */
export function createDiffPreviewModal(
  originalText,
  newText,
  onApprove,
  onCancel,
