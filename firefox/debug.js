...
(function() {
  ...
  const isPRPage = window.location.href.includes('/pull/') || window.location.href.includes('/compare/');
  ...
  if (typeof browser !== 'undefined') {
    ...
    browser.storage.sync.get(['figmaToken']).then(result => {
      console.log('Token configured:', !!result.figmaToken);
      if (result.figmaToken) {
        console.log('Token starts with:', result.figmaToken.substring(0, 10) + '...');
      }
    }).catch(err => {
